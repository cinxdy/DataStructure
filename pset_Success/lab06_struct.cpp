...
*
* Compilation : g++ struct.cpp -I ../../include -o struct
*
* History:
...
void step1() {
	...
	//cout << "Your code here\n";
	for (int i = 0; i < NCARS; i++) {
		cout << "\t(" << i+1 << ") " << list[i].model << "\t" << list[i].speed << endl;
	}
...
void step3(const int nCars) {
	...
	for (int i = 0; i < nCars; i++,p++) {
		cout << "[" << i + 1 << "/" << nCars << "]\n";
		//cout << "Your code here\n";
		p->model = GetString("\tEnter a model: ");
		p->speed = GetDouble("\tEnter a speed: ");
	}

	// get a copy of the pointer list; 
	// save the original pointer to use later.
	// cout << "Your code here\n";
	p = list;
